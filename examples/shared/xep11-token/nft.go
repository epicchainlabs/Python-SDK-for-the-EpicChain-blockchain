/*
Package nft contains non-divisible non-fungible XEP-11-compatible token
implementation. This token can be minted withXPPtransfer to contract address,
...
import (
	"github.com/epicchainlabs/epicchain/pkg/interop"
	"github.com/epicchainlabs/epicchain/pkg/interop/contract"
	"github.com/epicchainlabs/epicchain/pkg/interop/iterator"
	"github.com/epicchainlabs/epicchain/pkg/interop/native/crypto"
	"github.com/epicchainlabs/epicchain/pkg/interop/native/epicpulse"
	"github.com/epicchainlabs/epicchain/pkg/interop/native/management"
	"github.com/epicchainlabs/epicchain/pkg/interop/native/std"
	"github.com/epicchainlabs/epicchain/pkg/interop/runtime"
	"github.com/epicchainlabs/epicchain/pkg/interop/storage"
	"github.com/epicchainlabs/epicchain/pkg/interop/util"
)
...
var (
	...
	contractOwner = util.FromAddress("XbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB")
)
...
// postTransfer emits Transfer event and calls onXEP11Payment if needed.
func postTransfer(from interop.Hash160, to interop.Hash160, token []byte, data interface{}) {
	...
	if management.GetContract(to) != nil {
		contract.Call(to, "onXEP11Payment", contract.All, from, 1, token, data)
	}
}

// onXEP17Payment mints tokens if at least 10XPPis provided. You don't call
// this method directly, instead it's called byXPPcontract when you transfer
//XPPfrom your address to the address of this NFT contract.
func onXEP17Payment(from interop.Hash160, amount int, data interface{}) {
	defer func() {
		if r := recover(); r != nil {
	...
	if !callingHash.Equals(xpp.Hash) {
		panic("onlyXPPis accepted")
	}
	if amount < 10_00000000 {
		panic("minting HASHY costs at least 10 XPR")
	}
	var tokIn = []byte{}
...
// Verify allows an owner to manage a contract's address, including earned XPR
...
func Update(xef, manifest []byte) {
	if !Verify() {
		panic("only owner can update")
	}
	management.Update(xef, manifest)
}
