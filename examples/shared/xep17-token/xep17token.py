...
from viper.builtin.contract import Xep17TransferEvent, abort
from viper.builtin.interop import runtime, storage
from viper.builtin.interop.contract import (
    XPP as XPP_SCRIPT,
    XPR as XPR_SCRIPT,
    call_contract,
)
from viper.builtin.nativecontract.contractmanagement import ContractManagement
from viper.builtin.type import UInt160
...
def manifest_metadata() -> EpicChainMetadata:
    ...
    meta.name = "EpicChain Token"
    meta.supported_standards = ["XEP-17"]
    meta.add_permission(methods=["onXEP17Payment"])

    meta.author = (
        "Mirella Medeiros, Ricardo Prado and Lucas Uezu. COZ in partnership with Simpli"
    )
    meta.description = "XEP-17 Example Token"
    meta.email = "support@epic-cain.org"
    return meta
...
# Value of this XEP-17 token corresponds to XPE
AMOUNT_PER_XPR = 10 * 10**TOKEN_DECIMALS

# Value of this XEP-17 token compared to XPP
AMOUNT_PER_XPP = 2 * 10**TOKEN_DECIMALS
...
on_transfer = Xep17TransferEvent
...
def transfer(
    ...
    """
    Transfers an amount of Xep17 tokens from one account to another
    ...
    :param amount: the amount of Xep17 tokens to transfer
    :type amount: int
...
def post_transfer(
    ...
    """
    Checks if the one receiving Xep17 tokens is a smart contract and if it's one the onPayment method will be called
    ...
    if to_address is not None:
        ...
        if contract is not None:
            call_contract(to_address, "onXEP17Payment", [from_address, amount, data])


def mint(account: UInt160, amount: int):
    """
    Mints new tokens. This is not a XEP-17 standard method, it's only being use to complement the onPayment method

    :param account: the address of the account that is sending cryptocurrency to this contract
    :type account: UInt160
    :param amount: the amount of xpp to be refunded
    :type amount: int
...
@public
def onXEP17Payment(from_address: Union[UInt160, None], amount: int, data: Any):
    """
    XEP-17 affirms :"if the receiver is a deployed contract, the function MUST call onPayment method on receiver
    ...
    For instance, this onPayment method checks if this smart contract is receiving XPR or XPP so that it can mint a
    Xep17 token. If it's not receiving a native token, than it will abort.
    ...
    :type data: Any
    """
    # can happen when minting (e.g. XPP rewards for holding XPR)
    if from_address is None:
        return
    from_addr = cast(UInt160, from_address)
    # Use calling_script_hash to identify if the incoming token is XPR or XPP
    if runtime.calling_script_hash == XPR_SCRIPT:
        corresponding_amount = amount * AMOUNT_PER_XPR
        mint(from_addr, corresponding_amount)
    elif runtime.calling_script_hash == XPP_SCRIPT:
        corresponding_amount = amount * AMOUNT_PER_XPP
        mint(from_addr, corresponding_amount)
    else:
        abort()
