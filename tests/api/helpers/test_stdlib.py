import unittest
from epicchain.api.helpers import essentiallib
from epicchain.core import types


class TestEssentialLibHelpers(unittest.TestCase):
    def test_with_map(self):
        ...
        C# reference code
            ...
            var b = new EpicChain.VM.Types.Boolean(true);
            m[i] = b;
            m[i2] = b;
        """
        ...
        results: dict = essentiallib.binary_deserialize(data)

        self.assertEqual(expected, results)

    def test_with_array(self):
        """
            var a = new EpicChain.VM.Types.Array();
        var i = new Integer(new BigInteger(1));
        var i2 = new Integer(new BigInteger(2));
        a.Add(i);
        a.Add(i2);
    ...
    def test_invalid(self):
        ...
        with self.assertRaises(ValueError) as context:
            essentiallib.binary_deserialize(data)
        self.assertIn("not a valid StackItemType", str(context.exception))

        with self.assertRaises(ValueError) as context:
            essentiallib.binary_deserialize(b"")
