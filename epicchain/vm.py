"""
EpicChain Virtual Machine classes.
"""
from __future__ import annotations
import hashlib
from enum import IntEnum
from epicchain.contracts import callflags
from epicchain.core import types, serialization, cryptography
...
class OpCode(IntEnum):
    """
    EpicChain Virtual Machine instructions.

    Can be concatenated into a hex-escaped bytes sequence.

    Example:
        In [1]: from epicchain.contracts import vm
...
class Syscalls:
    """
    Container holding all EpicChain blockchain interop syscalls.
    """
    ...
    #: Get the name of the current platform. For EpicChain blockchain fixed to "EpicChain".
    ...
    #: Get the remainingXPPthat can be spent in order to complete the execution.
    SYSTEM_RUNTIME_EPICPULSE_LEFT = Syscall(
        "System.Runtime.EpicPulseLeft", callflags.CallFlags.NONE
    )
    #: Burns epicpulse.
    SYSTEM_RUNTIME_BURN_EPICPULSE = Syscall(
        "System.Runtime.BurnEpicPulse", callflags.CallFlags.NONE
    )
