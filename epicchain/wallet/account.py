...
from epicchain import settings, vm
from epicchain.contracts import abi, utils as contractutils, contract
from epicchain.network.payloads import transaction, verification
from epicchain.core import types, utils as coreutils, cryptography
from epicchain.wallet import utils, scrypt_parameters as scrypt
from epicchain.wallet.types import EpicChainAddress

# both constants below are used to encrypt/decrypt a private key to/from a xep2 key
XEP_HEADER = bytes([0x01, 0x42])
XEP_FLAG = bytes([0xE0])
...
class Account:
    ...
    def __init__(
        ...
        address: Optional[EpicChainAddress] = None,
        ...
        if watch_only:
            ...
            encrypted_key = self.private_key_to_xep2(
        ...
        self.address: EpicChainAddress = address
    ...
    def sign(self, data: bytes, password: str) -> bytes:
        ...
        private_key = self.private_key_from_xep2(self.encrypted_key.decode("utf-8"), password, _scrypt_parameters=self.scrypt_parameters)  # type: ignore
    ...
    def create_new(
        ...
        Args:
            password: the password to decrypt the xep2 key.
    ...
    def from_encrypted_key(
        ...
        Default settings assume a XEP-2 encrypted key.
        ...
        return cls(
            password=password,
            private_key=cls.private_key_from_xep2(
    ...
    def from_wif(
        ...
        Args:
            ...
            _scrypt_parameters: the Scrypt parameters to use to encode the private key. Default conforms to XEP-2.
    ...
    @classmethod
    def watch_only_from_address(cls, address: EpicChainAddress) -> Account:
        """
    ...
    def from_json(
        ...
        Args:
            ...
            scrypt_parameters: the Scrypt parameters to use to encode the private key. Default conforms to XEP-2.
    ...
    @staticmethod
    def private_key_from_xep2(
        xep2_key: str,
        passphrase: str,
        _scrypt_parameters: Optional[scrypt.ScryptParameters] = None,
    ) -> bytes:
        """
        Decrypt a xep2 key into a private key.

        Args:
            xep2_key: the key that will be decrypted.
            passphrase: the password to decrypt the xep2 key.
            _scrypt_parameters: a ScryptParameters object that will be passed to the key derivation function.

        Raises:
            ValueError: if the length of the xep2_key is not valid.
            ValueError: if it's not possible to decode the xep2_key.
        ...
        if len(xep2_key) != 58:
            raise ValueError(
                f"Please provide a xep2_key with a length of 58 bytes (LEN: {len(xep2_key)})"
            )

        address_hash_size = 4
        address_hash_offset = len(XEP_FLAG) + len(XEP_HEADER)

        try:
            decoded_key = base58.b58decode_check(xep2_key)
        except Exception:
            raise ValueError("Base58decode failure of xep2 key")
    ...
    @staticmethod
    def private_key_to_xep2(
        private_key: bytes,
        passphrase: str,
        _scrypt_parameters: Optional[scrypt.ScryptParameters] = None,
    ) -> bytes:
        """
        Encrypt a private key into a xep2 key.

        Args:
            private_key: the key that will be encrypted.
            passphrase: the password to encrypt the xep2 key.
            _scrypt_parameters: a ScryptParameters object that will be passed to the key derivation function.

        Returns:
            the encrypted xep2 key.
        """
        if _scrypt_parameters is None:
            _scrypt_parameters = scrypt.ScryptParameters()
        ...
        address = utils.script_hash_to_address(script_hash)
        # XEP2 checksum: hash the address twice and get the first 4 bytes
        first_hash = hashlib.sha256(address.encode("utf-8")).digest()
        second_hash = hashlib.sha256(first_hash).digest()
        checksum = second_hash[:4]
        ...
        xep2 = bytearray()
        xep2.extend(XEP_HEADER)
        xep2.extend(XEP_FLAG)
        xep2.extend(checksum)
        xep2.extend(encrypted)

        # Finally, encode with Base58Check
        encoded_xep2 = base58.b58encode_check(bytes(xep2))
