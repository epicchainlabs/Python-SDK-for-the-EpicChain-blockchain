...
from epicchain.network import message, capabilities, relaycache
from epicchain.network.ipfilter import ipfilter
from epicchain.network.convenience import nodeweight
from epicchain.network.payloads import address, version, inventory, ping, block
from epicchain import network_logger as logger, settings
from epicchain.core import types, msgrouter
from contextlib import suppress
from socket import AF_INET as IP4_FAMILY
from typing import Optional, Callable, cast
from asyncio.streams import StreamWriter, StreamReader
from epicchain.network.message import Message
...
class EpicChainNode:
    ...
    def connection_made(self) -> None:
        """
        Event called by the EpicChainNode.connect_to.
        """
    ...
    def handler_inv(self, msg: message.Message) -> None:
        ...
        if payload.type == inventory.InventoryType.BLOCK:
            # epicchain-cli broadcasts INV messages on a regular interval. We can use those as trigger to request
    ...
    def start_message_handler(self) -> None:
        ...
        configured in :attr:`~epicchain.network.node.EpicChainNode.dispatch_table`.
        """
    ...
    async def request_blocks(
        ...
        Note:
            The remote node is expected to reply with a Message with the :const:`~epicchain.network.message.MessageType.INV`
            type containing the hashes of the requested blocks. Use :meth:`~epicchain.network.node.EpicChainNode.request_data` in
            combination with these hashes to return the actual :class:`~epicchain.network.payloads.block.Block` objects.

        See also:
            `EpicChainNode.request_block_data()` to immediately retrieve
            `epicchain.network.payloads.block.Block` objects.
    ...
    async def request_block_data(self, index_start, count) -> None:
        ...
        Count cannot exceed :attr:`~epicchain.network.payloads.block.GetBlockByIndexPayload.MAX_BLOCKS_COUNT`.

        See also:
            :meth:`~epicchain.network.node.EpicChainNode.request_blocks()` to only request block hashes.
    ...
    async def connect_to(
        ...
        *,
    ...
    ) -> tuple[Optional[EpicChainNode], Optional[tuple[str, str]]]:
        """
        ...
        Returns:
            tuple[EpicChainNode, None]: if connection was established succesfully.
        ...
        try:
            ...
            node = EpicChainNode(reader, writer)
    ...
    async def _do_handshake(self) -> tuple[bool, Optional[address.DisconnectReason]]:
        ...
        send_version = message.Message(
            ...
            payload=version.VersionPayload(
                nonce=123, user_agent="EPICCHAIN_RAPTOR", capabilities=caps
            ),
        )
