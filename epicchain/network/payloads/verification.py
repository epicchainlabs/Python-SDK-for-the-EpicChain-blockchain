...
from epicchain.core import serialization, utils, types, cryptography, Size as s, interfaces
...
class WitnessScope(IntFlag):
    ...
    WITNESS_RULES = 0x40
    #: Allow the witness in all context. Equal to EpicChain 2.x's default behaviour.
    GLOBAL = 0x80
...
class IVerifiable(serialization.ISerializable):
    ...
    def get_hash_data(self, protocol_magic: int) -> bytes:
        ...
        Args:
            protocol_magic: network protocol number (EpicChain MainNet = 860833102, Testnet (T5) = 894710606, private net = ?)
        """
