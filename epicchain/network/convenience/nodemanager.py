...
import ipaddress
from epicchain import network_logger as logger, settings, singleton
from epicchain.network import node, message
from epicchain.network.convenience import requestinfo
from epicchain.network.payloads import address, ping
...
class NodeManager(singleton._Singleton):
    """
    Convenience class that helps establish and maintain a pool of active connections to EpicChain nodes.
    ...
    def init(self):
        ...
        self.nodes: list[node.EpicChainNode] = []
    ...
    def get_node_by_nodeid(self, nodeid: int) -> Optional[node.EpicChainNode]:
        """
    ...
    def get_node_with_height(self, height: int) -> Optional[node.EpicChainNode]:
        """
    ...
    def get_least_failed_node(
        ...
        for n in self.nodes:  # type: node.EpicChainNode
            if n.disconnecting:
                continue
    ...
    def increase_node_error_count(self, nodeid: int) -> None:
        ...
        Args:
            nodeid (`EpicChainNode.nodeid`): the specific node to update.
        """
    ...
    def _handle_node_connected(self, node: node.EpicChainNode) -> None:
    ...
    def _handle_node_disconnected(
        self, node: node.EpicChainNode, reason: address.DisconnectReason
    ) -> None:
    ...
    async def _process_seed_list_addresses(self) -> None:
        ...
        async def _process(seed):
            ...
            node.EpicChainNode.addresses.append(
    ...
    def _connect_done_cb(self, future) -> None:
        ...
        if failure:
            ...
            with suppress(ValueError):
                idx = node.EpicChainNode.addresses.index(tmp_addr)
                addr = node.EpicChainNode.addresses[idx]
                addr.set_state_dead()
    ...
    async def _fill_open_connection_spots(self) -> None:
        ...
        if open_spots > 0:
            ...
            node.EpicChainNode.addresses.sort(key=lambda addr: addr.last_connected)

            for _ in range(open_spots):
                # now we ask for the first address with the state NEW
                addr = node.EpicChainNode.get_address_new()
                if addr:
                    ...
                    if addr not in self.queued_addresses:
                        ...
                        if self._test_client_provider:
                            ...
                            task = asyncio.create_task(
                                node.EpicChainNode.connect_to(
                                    socket=r, _test_data=self._test_data
                                )
                            )
                        else:
                            task = asyncio.create_task(
                                node.EpicChainNode.connect_to(addr.ip, addr.port)
                            )
                    ...
                    if len(self.nodes) >= self.min_clients:
                        ...
                        if self.MAX_NODE_POOL_ERROR_COUNT != self.MAX_NODE_POOL_ERROR:
                            ...
                            for addr in node.EpicChainNode.addresses:
    ...
    async def _send_ping(self):
        ...
        EpicChain C# Preview3 no longer seems to instantly inform its connected nodes about new available blocks. Therefore,
        ...
        """
        for remote_node in self.nodes:  # type: node.EpicChainNode
