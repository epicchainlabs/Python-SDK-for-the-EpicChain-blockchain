...
class FlightInfo:
    ...
    tracking :class:`RequestInfo <epicchain.network.convenience.requestinfo.RequestInfo>` class.
    """

    def __init__(self, node_id: int, height: int):
        """
        Args:
            node_id: the :attr:`~epicchain.network.node.EpicChainNode.id` of the node the data is requested from
            height: the header or block height being requested
        """

        #: The :attr:`~epicchain.network.node.EpicChainNode.id` of the node the data is requested from.
