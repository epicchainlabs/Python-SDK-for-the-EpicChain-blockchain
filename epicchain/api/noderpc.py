...
from epicchain.core import types, cryptography, interfaces, serialization
from epicchain.contracts import manifest, xef, contract, abi
...
class GetVersionResponse:
    ...
    def from_json(cls, json: dict):
        ...
        vp = VersionProtocol(
            ...
            p["initialepicpulsedistribution"],
        )
...
@dataclass
class Xep17BalancesResponse:
    """
    Response to `getxep17balances` RPC call.
    """

    balances: list[Xep17Balance]
    address: str
    ...
    def from_json(cls, json: dict):
        ...
        for b in json["balance"]:
            ...
            a = int(b["amount"])
            c.balances.append(Xep17Balance(h, a, b["lastupdatedblock"]))
        return c
...
@dataclass
class Xep17TransfersResponse:
    """
    Response to `getXep17Transfer` RPC call.
    """

    sent: list[Xep17Transfer]
    received: list[Xep17Transfer]
    address: str
    ...
    def from_json(cls, json: dict):
        c = cls([], [], json["address"])
        for t in json["sent"]:
            c.sent.append(Xep17Transfer.from_json(t))
        for t in json["received"]:
            c.received.append(Xep17Transfer.from_json(t))
        return c
...
class EpicRpcClient(RPCClient):
    ...
    async def get_contract_state(
        ...
        xef_ = xef.XEF.from_json(result["xef"])
        manifest_ = manifest.ContractManifest.from_json(result["manifest"])
        return contract.ContractState(
            result["id"], xef_, manifest_, result["updatecounter"], h
        )

    async def get_xep17_balances(self, address: str) -> Xep17BalancesResponse:
        """
        Fetch the balance of all Xep17 assets for the specified address.
        """
        result = await self._do_post("getXep17Balances", [address])
        return Xep17BalancesResponse.from_json(result)

    async def get_xep17_transfers(
        self,
        address: str,
        start_time: Optional[datetime.datetime] = None,
        end_time: Optional[datetime.datetime] = None,
    ) -> Xep17TransfersResponse:
        """
        Fetch Xep17 transfers for a given address. Defaults to the last 7 days on the server side.
        ...
        Example:
            ...
            await get_Xep17_transfers(<your address>, start)
        """
        params = [address]
        ...
        result = await self._do_post("getXep17Transfers", params)
        return Xep17TransfersResponse.from_json(result)
    ...
    async def get_unclaimed_xpp(self, address: str) -> int:
        """
        Fetch the amount of unclaimed xpp for the given address.

        Args:
            address: a EpicChain address.
        """
        result = await self._do_post("getunclaimedepicchain", [address])
