...
The most specific wrappers in this module are for EpicChains native contracts `EpicChain`, `EpicPulse`, `PolicyContract` and
 `RoleManagement`. The `ContractManagement` and `QuantumVaultAsset` contracts are not wrapped. See the FAQ for the reasons.

One step up are wrappers for contracts following the XEP-17 Token standard (`XEP17Contract`) and XEP-11 NFT standard
(`XEP11DivisibleContract` & `XEP11NonDivisibleContract`).
...
1. `test_invoke()` - does not persist to chain. Costs no xpp.
2. `invoke()` - does persist to chain. Requires signing and costs xpp.
3. `estimate_xpp()` - does not persist to chain. Costs no xpp.
...
Example:
    ...
    result = await facade.test_invoke(epicchain.candidates_registered())
"""
...
from epicchain.api import noderpc
from epicchain.api.helpers import signing, txbuilder, unwrap
from epicchain.network.payloads import verification
from epicchain.wallet import utils as walletutils
from epicchain.wallet.types import EpicChainAddress
from epicchain.core import types, cryptography, utils as coreutils
from epicchain import vm
from epicchain.contracts import contract, callflags, utils as contractutils, xef, manifest
...
class InvokeReceipt(Generic[ReturnType]):
    ...
    confirmations: int
    #: The total xpp cost for block inclusion and script execution.
    xpp_consumed: int
    ...
    def __repr__(self):
        return (
            ...
            f"confirmations={self.confirmations}, xpp_consumed={self.xpp_consumed}, state={self.state}, "
...
class ChainFacade:
    ...
    def __init__(
        ...
        """
        Args:
            rpc_host: EpicChain RPC node host address.
    ...
    async def test_invoke(
        ...
        *,
        ...
        This does not persist any state on the actual chain and therefore does not require signing or paying XPP.
    ...
    async def invoke(
        ...
        *,
        ...
        """
        Call a contract method and persist results on the chain. Costs XPP.
        ...
        async with noderpc.EpicRpcClient(self.rpc_host) as client:
            ...
            return InvokeReceipt[ReturnType](
                ...
                receipt.execution.xpp_consumed,
    ...
    async def invoke_multi(
        ...
        *,
        ...
        """
        Call all contract methods (concatenated) in one go and persist results on the chain. Costs XPP.
        Waits for tx to be included in a block. Automatically post processes the execution results according to the
        post-processing function of `f` if present.
    ...
    async def estimate_xpp(
        ...
        *,
        ...
        """
        Estimate the xpp price for calling the contract method.
        """
        async with noderpc.EpicRpcClient(self.rpc_host) as client:
            res = await client.invoke_script(f.script, signers)
            return res.xpp_consumed
...
class GenericContract:
    ...
    def update(
        ...
        xef: Optional[xef.XEF] = None,
        manifest: Optional[manifest.ContractManifest] = None,
        data: Optional[noderpc.ContractParameter] = None,
    ) -> ContractMethodResult[None]:
        """
        Update this contract on chain with a new manifest and/or contract (XEF).

        Assumes the update method on the contract uses the standard arguments; xef, manifest and (optional) data.
        ...
        Args:
            ...
            xef: compiled contract.
            manifest: contract manifest.
            data: data that is passed to the `_deploy` method of the smart contract (if the method exists).
        """
        if xef is None and manifest is None:
            raise ValueError("XEF and manifest are both None. Nothing to update")
        ...
        sb = vm.ScriptBuilder()
        sb.emit_contract_call_with_args(self.hash, update_method, [xef, m, data])
    ...
    def deploy(
        ...
        sb.emit_contract_call_with_args(
            contract.CONTRACT_HASHES.MANAGEMENT, "deploy", [xef, str(manifest), data]
        )
...
class _TokenContract(GenericContract):
    ...
    def total_supply(self) -> ContractMethodResult[int]:
        """
        Get the total token supply in the EpicChain system.
        """
        script = (
...
class XEP17Contract(_TokenContract):
    """
    Base class for calling XEP-17 compliant smart contracts.
    """

    def balance_of(
        self, account: types.UInt160 | EpicChainAddress
    ) -> ContractMethodResult[int]:
        """
        Get the balance for the given account.

        Note:
            The returned value does not take the token decimals into account. e.g. for theXPPtoken you want to
        divide the result by 10**8 (as the EpicPulse token has 8 decimals).

        Raises:
            ValueError: if `account` is an invalid EpicChainAddress format.
        """
    ...
    def transfer(
        self,
        source: types.UInt160 | EpicChainAddress,
        destination: types.UInt160 | EpicChainAddress,
        amount: int,
        data: Optional[noderpc.ContractParameter] = None,
    ) -> ContractMethodResult[bool]:
        """
        Transfer `amount` of tokens from `source` account to `destination` account.
        Forward `data` to `onXEP17Payment` handler if applicable.
        ...
        Raises:
            ValueError: if `source` or `destination` is an invalid EpicChainAddress format
    ...
    def transfer_multi(
        self,
        source: types.UInt160 | EpicChainAddress,
        destinations: Sequence[types.UInt160 | EpicChainAddress],
        amount: int,
        data: Optional[noderpc.ContractParameter] = None,
        ...
        Args:
            ...
            data: forward to `onXEP17Payment` handler if applicable.
            abort_on_failure: if True aborts the whole transaction if any of the transfers fails.

        Raises:
            ValueError: if any of the destinations is supplied as EpicChainAddress but is an invalid address format.
...
class EpicPulse(XEP17Contract):
    """
    WrappedXPPtoken contract.
    """

    def __init__(self):
        super(EpicPulse, self).__init__(contract.CONTRACT_HASHES.EPICPULSE_TOKEN)
...
class Candidate:
    ...
    def __init__(self, public_key: cryptography.ECPoint, votes: int):
        ...
        #: EpicChain address of the candidate.
        self.address = walletutils.script_hash_to_address(shash)


class EpicChain(XEP17Contract):
    """
    Wrapped EpicChain token contract.
    """

    def __init__(self):
        super(EpicChain, self).__init__(contract.CONTRACT_HASHES.EPICCHAIN_TOKEN)

    def get_xpp_per_block(self) -> ContractMethodResult[int]:
        """
        Get the amount ofXPPgenerated in each block.
        """
        script = (
            vm.ScriptBuilder()
            .emit_contract_call(self.hash, "getEpicPulsePerBlock")
            .to_array()
        )
        return ContractMethodResult(script, unwrap.as_int)

    def get_unclaimed_epicpulse(
        self, account: types.UInt160 | EpicChainAddress, end: Optional[int] = None
    ) -> ContractMethodResult[int]:
        """
        Get the amount of unclaimed XPP.

        Args:
            account: for whom.
            end: up to which block height to calculate theXPPbonus. Omit to calculate to the current chain height.
        ...
        if end is not None:
            sb.emit_contract_call_with_args(
                self.hash, "unclaimedEpicPulse", [account, end]
            ).to_array()
            return ContractMethodResult(sb.to_array(), unwrap.as_int)
        else:
            sb.emit_contract_call(contract.CONTRACT_HASHES.QUANTUMVAULTASSET, "currentIndex")
            # first argument to "unclaimedEpicPulse" (second is already on the stack by the "currentIndex" call
            sb.emit_push(account)
            # length of arguments
            sb.emit_push(2)
            sb.emit(vm.OpCode.PACK)
            sb.emit_push(callflags.CallFlags.ALL)
            sb.emit_push("unclaimedEpicPulse")
            sb.emit_push(self.hash)
            sb.emit_syscall(vm.Syscalls.SYSTEM_CONTRACT_CALL)
            return ContractMethodResult(sb.to_array(), unwrap.as_int)
    ...
    def candidate_registration_price(self) -> ContractMethodResult[int]:
        """
        Get the amount ofXPPto pay to register as consensus candidate.
        """
        script = (
            vm.ScriptBuilder()
    ...
    def candidate_vote(
        self, voter: types.UInt160 | EpicChainAddress, candidate: cryptography.ECPoint
    ) -> ContractMethodResult[bool]:
        """
        ...
        Raises:
            ValueError: if `voter` is an invalid EpicChainAddress format.
...
class _XEP11Contract(_TokenContract):
    """
    Base class for calling XEP-11 compliant smart contracts.

    NFTs can be divisible or non-divisible which is determined by the value of `decimals()`.

    Note:
        The following 2 common methods defined in the XEP-11 standard have different names to improve discoverability
         XEP-11 standard    This wrapper
    ...
    def decimals(self) -> ContractMethodResult[int]:
        ...
        """
        return super(_XEP11Contract, self).decimals()

    def total_owned_by(
        self, owner: types.UInt160 | EpicChainAddress
    ) -> ContractMethodResult[int]:
        """
        Get the total amount of NFTs owned for the given account.

        Raises:
            ValueError: if `owner` is an invalid EpicChainAddress format.
        """
    ...
    def tokens(
        ...
        Note2:
            This is an optional method in the XEP-11 standard and can thus fail.
        """
...
class XEP11DivisibleContract(_XEP11Contract):
    """
    Base class for divisible NFTs.

    The XEP-11 `ownerOf` method is named `owners_of` in this wrapper.
    """
    ...
    def balance_of(
        self, owner: types.UInt160 | EpicChainAddress, token_id: bytes
    ) -> ContractMethodResult[int]:
        """
...
class XEP11NonDivisibleContract(_XEP11Contract):
    ...
    def transfer(
        ...
        Raises:
            ValueError: if `destination` is an invalid EpicChainAddress format.
...
def _check_address_and_convert(value: types.UInt160 | EpicChainAddress) -> types.UInt160:
    if isinstance(value, types.UInt160):
        return value
    if not isinstance(value, str):
        raise ValueError(
            f"Input is of type {type(value)} expected UInt160 or EpicChainAddress(str)"
        )
...
class RoleContract(GenericContract):
    ...
    def __init__(self):
        super(RoleContract, self).__init__(contract.CONTRACT_HASHES.QUANTUMGUARDNEXUS)
